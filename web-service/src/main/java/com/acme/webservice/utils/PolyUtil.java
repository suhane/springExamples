...
import static com.acme.webservice.utils.SphericalUtil.*;
...
public class PolyUtil {
    ...
    public static boolean containsLocation(Coordinates point, List<Coordinates> polygon, boolean geodesic) {
        return containsLocation(point.getLat(), point.getLng(), polygon, geodesic);
    }
     ...
     */
    public static boolean containsLocation(double latitude, double longitude, List<Coordinates> polygon, boolean geodesic) {
     ...
     */
    public static boolean isLocationOnEdge(Coordinates point, List<Coordinates> polygon, boolean geodesic,
                                           double tolerance) {
        return isLocationOnEdgeOrPath(point, polygon, true, geodesic, tolerance);
    }

    /**
     * Same as {@link #isLocationOnEdge(Coordinates, List, boolean, double)}
     ...
     */
    public static boolean isLocationOnPath(Coordinates point, List<Coordinates> polyline,
                                           boolean geodesic, double tolerance) {
        return isLocationOnEdgeOrPath(point, polyline, false, geodesic, tolerance);
    }

    /**
     * Same as {@link #isLocationOnPath(Coordinates, List, boolean, double)}
     *
     * with a default tolerance of 0.1 meters.
     */
    public static boolean isLocationOnPath(Coordinates point, List<Coordinates> polyline,
    ...
    private static boolean isLocationOnEdgeOrPath(Coordinates point, List<Coordinates> poly, boolean closed,
     ...
     */
    public static int locationIndexOnPath(Coordinates point, List<Coordinates> poly,
                                          boolean geodesic, double tolerance) {
        return locationIndexOnEdgeOrPath(point, poly, false, geodesic, tolerance);
    }

    /**
     * Same as {@link #locationIndexOnPath(Coordinates, List, boolean, double)}
     *
     * with a default tolerance of 0.1 meters.
     */
    public static int locationIndexOnPath(Coordinates point, List<Coordinates> polyline,
                                          boolean geodesic) {
     ...
     *          poly.size()-2 if between poly[poly.size() - 2] and poly[poly.size() - 1]
     */
    private static int locationIndexOnEdgeOrPath(Coordinates point, List<Coordinates> poly, boolean closed,
                                                 boolean geodesic, double toleranceEarth) {
        int size = poly.size();
        if (size == 0) {
            return -1;
        ...
        Coordinates prev = poly.get(closed ? size - 1 : 0);
        double lat1 = toRadians(prev.getLat());
        double lng1 = toRadians(prev.getLng());
        int idx = 0;
        if (geodesic) {
            for (Coordinates point2 : poly) {
                double lat2 = toRadians(point2.getLat());
                double lng2 = toRadians(point2.getLng());
     ...
     */
    public static List<Coordinates> simplify(List<Coordinates> poly, double tolerance) {
        ...
        if (closedPolygon) {
            ...
            // Coordinates.getLat() and .getLng() are immutable, so replace the last point
            poly.remove(poly.size() - 1);
            poly.add(new Coordinates(lastPoint.getLat() + OFFSET, lastPoint.getLng() + OFFSET));
        }
        ...
        idx = 0;
        ArrayList<Coordinates> simplifiedLine = new ArrayList<>();
        for (Coordinates l : poly) {
            if (dists[idx] != 0) {
     ...
     * points are the same), and false if it is not
     */
    public static boolean isClosedPolygon(List<Coordinates> poly) {
        Coordinates firstPoint = poly.get(0);
        Coordinates lastPoint = poly.get(poly.size()-1);
     ...
     */
    public static double distanceToLine(final Coordinates p, final Coordinates start, final Coordinates end) {
        if (start.equals(end)) {
            return computeDistanceBetween(end, p);
        }

        final double s0lat = toRadians(p.getLat());
        final double s0lng = toRadians(p.getLng());
        final double s1lat = toRadians(start.getLat());
        final double s1lng = toRadians(start.getLng());
        final double s2lat = toRadians(end.getLat());
        final double s2lng = toRadians(end.getLng());
        ...
        Coordinates sa = new Coordinates(p.getLat() - start.getLat(), p.getLng() - start.getLng());
        Coordinates sb = new Coordinates(u * (end.getLat() - start.getLat()), u * (end.getLng() - start.getLng()));
        return computeDistanceBetween(sa, sb);
    }

    /**
     * Decodes an encoded path string into a sequence of Coordinatess.
     */
    public static List<Coordinates> decode(final String encodedPath) {
        ...
        while (index < len) {
            ...
            path.add(new Coordinates(lat * 1e-5, lng * 1e-5));
        }
    ...
    /**
     * Encodes a sequence of Coordinatess into an encoded path string.
     */
    public static String encode(final List<Coordinates> path) {
        long lastLat = 0;
        long lastLng = 0;

        final StringBuffer result = new StringBuffer();

        for (final Coordinates point : path) {
